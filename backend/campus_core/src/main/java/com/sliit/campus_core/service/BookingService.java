...
import com.sliit.campus_core.entity.*;
import com.sliit.campus_core.exception.*;
import com.sliit.campus_core.repository.*;
...
public class BookingService {
    ...
    public List<BookingResponse> getAllBookings(BookingStatus status, LocalDate date, String resourceId) {
        List<Booking> bookings = bookingRepository.findAll().stream()
                ...
                .filter(booking -> resourceId == null || resourceId.isBlank()
                        || resourceId.equals(booking.getResource().getId()))
    ...
    private void validateResource(Resource resource) {
        ...
        if (resource.getAvailabilityWindow() == null || resource.getAvailabilityWindow().isEmpty()) {
            throw new ResourceNotAvailableException(
                    "Resource availability hours are not configured: " + resource.getName());
        }
    }
    
    private void validateBookingTime(Resource resource, LocalTime startTime, LocalTime endTime) {
        String[] times = resource.getAvailabilityWindow() != null ? 
            resource.getAvailabilityWindow().split("-") : new String[]{"08:00", "18:00"};
            
        LocalTime resStart = LocalTime.parse(times.length > 0 ? times[0].trim() : "08:00");
        LocalTime resEnd = LocalTime.parse(times.length > 1 ? times[1].trim() : "18:00");

        if (startTime.isBefore(resStart) || endTime.isAfter(resEnd)) {
    ...
    private byte[] generateQRCode(Booking booking) {
        try {
            String qrContent = String.format("Booking ID: %s, Resource: %s, Date: %s, Time: %s - %s",
                    booking.getId(),
    ...
    private ResourceResponse convertToResourceResponse(Resource resource) {
        ...
        // Parse availability window to start/end time
        String[] times = resource.getAvailabilityWindow() != null ? 
            resource.getAvailabilityWindow().split("-") : new String[]{"08:00", "18:00"};
            
        response.setStartTime(times.length > 0 ? times[0].trim() : "08:00");
        response.setEndTime(times.length > 1 ? times[1].trim() : "18:00");
